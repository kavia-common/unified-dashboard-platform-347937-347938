...
class UserGoal(Base):
    ...
    user_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Exercise(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    created_by: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="SET NULL"), nullable=True)

    name: Mapped[str] = mapped_column(Text, nullable=False)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    primary_muscle_group: Mapped[str | None] = mapped_column(Text, nullable=True)
    secondary_muscle_groups: Mapped[list[str]] = mapped_column(ARRAY(Text), nullable=False, server_default="{}")
    ...
    instructions: Mapped[list] = mapped_column(JSONB, nullable=False, server_default="[]")
    media: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default="{}")
...
class WorkoutTemplate(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    owner_user_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="CASCADE"), nullable=True)
...
class WorkoutTemplateExercise(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    workout_template_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("workout_template.id", ondelete="CASCADE"), nullable=False)
    exercise_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("exercise.id", ondelete="RESTRICT"), nullable=False)

    position: Mapped[int] = mapped_column(Integer, nullable=False)
    sets: Mapped[int | None] = mapped_column(Integer, nullable=True)
    reps: Mapped[int | None] = mapped_column(Integer, nullable=True)
    rep_range: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default="{}")
...
class PlannedWorkoutSession(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    workout_plan_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("workout_plan.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="CASCADE"), nullable=False, index=True)

    scheduled_date: Mapped[object] = mapped_column(Date, nullable=False)
    title: Mapped[str | None] = mapped_column(Text, nullable=True)
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    workout_template_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("workout_template.id", ondelete="SET NULL"), nullable=True)
...
class PlannedSessionExercise(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    planned_session_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("planned_workout_session.id", ondelete="CASCADE"), nullable=False)
    exercise_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("exercise.id", ondelete="RESTRICT"), nullable=False)
...
class WorkoutLog(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    user_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="CASCADE"), nullable=False, index=True)
    planned_session_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("planned_workout_session.id", ondelete="SET NULL"), nullable=True)
...
class WorkoutLogExercise(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    workout_log_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("workout_log.id", ondelete="CASCADE"), nullable=False)
    exercise_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("exercise.id", ondelete="RESTRICT"), nullable=False)
...
class WorkoutLogSet(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    workout_log_exercise_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("workout_log_exercise.id", ondelete="CASCADE"), nullable=False)
...
class PersonalRecord(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    user_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="CASCADE"), nullable=False, index=True)
    exercise_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("exercise.id", ondelete="SET NULL"), nullable=True)
...
class NotificationDelivery(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    schedule_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("notification_schedule.id", ondelete="SET NULL"), nullable=True)
    user_id: Mapped[str] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="CASCADE"), nullable=False, index=True)
...
class AppEvent(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    user_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), ForeignKey("app_user.id", ondelete="SET NULL"), nullable=True)
